lnl.ordinal <- function(param, y, X, id, model, link, rn, gradient = FALSE, hessian = FALSE,
  ...
  repeat{
    ...
    mu <- c(-100, 0, param[(K+1L):(K+J-2L)] +
            direction[(K+1L):(K+J-2L)], 100)
    ...
    if (model == "random"){
      sigma <- param[K+J-1L] + step * direction[K+J-1L]
